...
int main() {
  ...
  vector <LL> v(1 << 19, n - 2), v2(1 << 19, n - 2);
  LazySegmentTree r(v), u(v2);
  LL ans = (n - 2) * (n - 2);
  for (int i = 0; i < q; i++) {
    if (pr[i].first == 1) {
      int g = r.find(pr[i].second - 2, pr[i].second - 1);
      ans -= g;
      u.update(0, g + 1, pr[i].second - 2);
    } else {
      int g = u.find(pr[i].second - 2, pr[i].second - 1);
      ans -= g;
      r.update(0, g + 1, pr[i].second - 2);
    }
  }
  cout << ans << endl;
  return 0;
