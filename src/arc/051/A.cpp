...
int main() {
  ...
  Circle c(p2(x1, y1), r);
  Poly p(vector<p2>({p2(x2, y2), p2(x2, y3), p2(x3, y3), p2(x3, y2)}));

  if (p.include(c.p, true, true) && p.include(p2(x1 + r, y1), true, true) && p.include(p2(x1 - r, y1), true, true) &&
      p.include(p2(x1, y1 + r), true, true) && p.include(p2(x1, y1 - r), true, true)) {
    cout << "NO" << endl;
  } else {
    cout << "YES" << endl;
  }

  if (c.include(p.ps[0]) && c.include(p.ps[1]) && c.include(p.ps[2]) && c.include(p.ps[3])) {
    cout << "NO" << endl;
  } else {
    cout << "YES" << endl;
  }
