...
tRMQ<pli> minrmq(int n) {
  return tRMQ<pli>(n, make_pair(-10101010, -1), [](pli r, pli l) { return max(l, r); });
}
...
int main() {
  ...
  auto r = minrmq(n);
  for (int i = 0; i < n; i++) r.update(i, make_pair(a[i], i));
  ll ans = 0;
  ll done = 0;
  while (done < n) {
    auto pr = r.query(max(0LL, n - done - 1), n);
    done += pr.first;
    ans++;
    r.update(pr.second, make_pair(0, pr.second));
  }
  cout << ans << endl;
  return 0;
}
