...
long long c;
vector<long long> h;
long long dp[202020] = {0};

template <typename T> class ConvecHullTrick {
private:
  // 直線群(配列)
...
public:
  ...
  ConvecHullTrick(bool flagX = false, std::function<bool(T l, T r)> compFunc =
                                          [](T l, T r) { return l >= r; })
...
// ref: http://satanic0258.hatenablog.com/entry/2016/08/16/181331
...
int main() {
  ...
  for (int i = 0; i < n; i++)
    cin >> h[i];
  ConvecHullTrick<long long> q(true);
  dp[0] = 0;
  q.add(-2 * h[0], h[0] * h[0] + dp[0]);
  for (int i = 1; i < n; i++) {
    dp[i] = q.get(h[i]) + h[i] * h[i] + c;
    q.add(-2 * h[i], h[i] * h[i] + dp[i]);
  }
  cout << dp[n - 1] << endl;
  return 0;
}
